...
export function DonutChart({ data = [], tamanho = 188, espessura = 24 }) {
  ...
  const gap = data.length > 1 ? circ * 0.012 : 0; // respiro entre fatias
  let acumulado = 0;
  return (
...
/** Gera um path suave (curva Catmull-Rom → Bézier) a partir de pontos {x,y}. */
function pathSuave(pts, tensao = 0.2) {
    ...
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block', height: 'auto', overflow: 'visible' }}>
      ...
      {data.map((d, i) => (
        <g key={i}>
          ...
          {mostrarValores && (
            <text x={x(i)} y={y(d.value) - 12} textAnchor="middle" className="mono" style={{ fontSize: '13px', fontWeight: 700, fill: 'var(--gray-900)' }}>{fmt(d.value)}</text>
          )}
          {i % passoRotulo === 0 && (
